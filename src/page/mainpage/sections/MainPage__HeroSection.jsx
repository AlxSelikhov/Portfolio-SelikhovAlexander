import styled from "styled-components";

const Text5 = styled.div`
position: relative;
line-height: 150%;
font-weight: 500;

@media (max-width: 1024px){
position: relative;
line-height: 150%;
font-weight: 500;
}
`;
const SubContainer4 = styled.div`
border-radius: 13px;
background-color: var(--peach-95);
display: flex;
flex-direction: row;
align-items: center;
justify-content: flex-start;
padding: 10px var(--padding-lg);

@media (max-width: 1200px){
    border-radius: 13px;
    background-color: var(--peach-95);
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    padding: 10px var(--padding-lg);
}
`;
const Heading = styled.div`
align-self: stretch;
position: relative;
line-height: 150%;
font-weight: 600;

@media (max-width: 1200px){
    align-self: stretch;
    position: relative;
    line-height: 150%;
    font-weight: 600;
}
`;
const Paragraph = styled.div`
align-self: stretch;
position: relative;
font-size: var(--font-size-lg);
line-height: 150%;
color: var(--grey-30);

@media (max-width: 1200px){
    align-self: stretch;
    position: relative;
    font-size: var(--font-size-lg);
    line-height: 150%;
    color: var(--grey-30);
}
`;
const TextContainer3 = styled.div`
align-self: stretch;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;
gap: 10px;
font-size: 58px;
color: var(--grey-10);

@media (max-width: 1200px){
    align-self: stretch;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: flex-start;
    gap: 10px;
    text-align: center;
    font-size: 58px;
    color: var(--grey-10);
}
`;
const Container3 = styled.div`
align-self: stretch;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;
gap: 20px;

@media (max-width: 1200px){
    width: 768px;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: flex-start;
    gap: 20px;
}
`;
const Button = styled.div`
border-radius: 8px;
background-color: var(--purple-50);
display: flex;
flex-direction: row;
align-items: center;
justify-content: flex-start;
padding: var(--padding-lg) 24px;

@media (max-width: 1200px){
    border-radius: 8px;
    background-color: var(--purple-50);
    display: flex;
    flex-direction: row;
    align-items: center;
    justify-content: flex-start;
    padding: var(--padding-lg) 24px;
}
`;
const Text7 = styled.div`
position: relative;
line-height: 150%;
font-weight: 500;
color: var(--grey-10);

@media (max-width: 1200px){
    position: relative;
    line-height: 150%;
    font-weight: 500;
    color: var(--grey-10);
}
`;
const ButtonsContainer1 = styled.div`
display: flex;
flex-direction: row;
align-items: center;
justify-content: flex-start;
gap: 40px;
font-size: var(--font-size-lg);
color: var(--peach-99);

@media (max-width: 1200px){
width: 100%;
position: relative;
display: flex;
flex-direction: row;
align-items: center;
justify-content: center;
gap: 40px;
text-align: left;
font-size: var(--font-size-lg);
color: var(--peach-99);
font-family: var(--font-manrope);
}
`;
const SubContainer3 = styled.div`
align-self: stretch;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;
padding: 0px 156px 0px 0px;
gap: 50px;

@media (max-width: 1200px){
    align-self: stretch;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    padding:0;
    gap: 50px;
}
`;
const Span2 = styled.span`
`;
const Span = styled.span`
color: var(--purple-50);

@media (max-width: 1024px){
    color: var(--purple-50);
}
`;
const Number1 = styled.b`
align-self: stretch;
position: relative;
line-height: 150%;

@media (max-width: 1200px){
    align-self: stretch;
position: relative;
line-height: 150%;
`;
const Text8 = styled.div`
align-self: stretch;
position: relative;
font-size: var(--font-size-lg);
line-height: 150%;
font-weight: 500;
color: var(--grey-30);

@media (max-width: 1200px){
    align-self: stretch;
    position: relative;
    font-size: var(--font-size-lg);
    line-height: 150%;
    font-weight: 500;
    color: var(--grey-30);
}
`;
const TextContainer4 = styled.div`
width: 727px;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;

@media (max-width: 1200x){
    width: 369px;
    display: flex;
    flex-direction: column;
    align-items: flex-start;
    justify-content: flex-start;
}
`;
const TextContainer5 = styled.div`
flex: 1;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;

@media (max-width: 1200px){
    width: 319px;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;
}
`;
const SubContainer5 = styled.div`
align-self: stretch;
border-radius: var(--br-xs) 0px 0px var(--br-xs);
background-color: var(--peach-96);
display: flex;
flex-direction: row;
align-items: flex-start;
justify-content: flex-start;
padding: 40px 50px;
gap: 30px;
font-size: var(--font-size-29xl);
color: var(--color-darkslategray);

@media (max-width: 1200px){
 display:none;
}
`;
const Container2 = styled.div`flex: 1;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;
gap: 80px;

@media (max-width: 1200px){
width: 768px;
display: flex;
flex-direction: column;
align-items: flex-start;
justify-content: flex-start;
gap: var(--gap-21xl);
}
`;
const HeroSectionRoot = styled.div`
position: relative;
width: 100%;
height: 630px;
display: flex;
flex-direction: row;
align-items: center;
justify-content: flex-start;
padding: 0px 0px 0px 120px;
box-sizing: border-box;
text-align: left;
font-size: 28px;
color: var(--grey-30);
font-family: var(--font-manrope);
margin-top: 100px;

@media (max-width: 1200px){
    position: relative;
    width: 100%;
    display: flex;
    flex-direction: column;
    align-items: center;
    justify-content: center;
    padding: 0px 128px;
    box-sizing: border-box;
    text-align: left;
    font-size: 28px;
    color: var(--grey-30);
    font-family: var(--font-manrope);
}
`;


const MainPage__HeroSection = () => {
    return (
        <HeroSectionRoot>
            <Container2>
                <SubContainer3>
                    <Container3>
                        <SubContainer4>
                            <Text5>Всем привет 🤗</Text5>
                        </SubContainer4>
                        <TextContainer3>
                            <Heading>Селихов Александр Приветствует Вас!</Heading>
                            <Paragraph>Я - преданный и опытный frontend разработчик, стремящийся создавать визуально привлекательные и высокофункциональные веб-приложения. Изучите мои проекты, чтобы увидеть, как эффективно я применяю свои навыки в разработке пользовательского интерфейса.</Paragraph>
                        </TextContainer3>
                    </Container3>
                    <ButtonsContainer1>
                        <Button>
                            <Text5>Скачать резюме</Text5>
                        </Button>
                        <Text7>{`Просмотреть портфолио ->`}</Text7>
                    </ButtonsContainer1>
                </SubContainer3>
                <SubContainer5>
                    <TextContainer4>
                        <Number1>
                            <Span2>5</Span2>
                            <Span>+</Span>
                        </Number1>
                        <Text8>Завершенных проектов (commercial + pet)</Text8>
                    </TextContainer4>
                    <TextContainer5>
                        <Number1>
                            <Span2>01</Span2>
                            <Span>+</Span>
                        </Number1>
                        <Text8>Лет опыта работы</Text8>
                    </TextContainer5>
                </SubContainer5>
            
                <MobileSubContainer1Root>
                    <MobileTextContainer2>
                        <MobileNumber>
                            <MobileSpan2>5</MobileSpan2>
                            <MobileSpan>+</MobileSpan>
                        </MobileNumber>
                        <MobileText2>Завершенных проектов (commercial + pet)</MobileText2>
                    </MobileTextContainer2>
                    <MobileTextContainer2>
                        <MobileNumber>
                            <MobileSpan2>01</MobileSpan2>
                            <MobileSpan>+</MobileSpan>
                        </MobileNumber>
                        <MobileText2>Лет опыта работы</MobileText2>
                    </MobileTextContainer2>
                </MobileSubContainer1Root>
            </Container2>
        </HeroSectionRoot>
    );
};

export default MainPage__HeroSection;


const MobileSpan2 = styled.span`
`;
const MobileSpan = styled.span`
@media (max-width: 1200px){
    color: var(--color-blue);
}
`;
const MobileNumber = styled.b`
@media (max-width: 1200px){
align-self: stretch;
position: relative;
line-height: 150%;
}
`;
const MobileText2 = styled.div`
@media (max-width: 1200px){
align-self: stretch;
position: relative;
font-size: var(--font-size-base);
line-height: 150%;
font-weight: 500;
color: var(--grey-30);
}
`;
const MobileTextContainer2 = styled.div`
@media (max-width: 1200px){
border-radius: var(--br-5xs);
background-color: var(--peach-96);
width: 768px;
display: flex;
flex-direction: column;
align-items: center;
justify-content: flex-start;
padding: var(--padding-base) var(--padding-lg);
box-sizing: border-box;
}
`;
const MobileSubContainer1Root = styled.div`

border-radius: var(--br-xs) 0px 0px var(--br-xs);
width: 768px;
display: flex;
flex-direction: column;
align-items: center;
justify-content: center;
gap: var(--gap-3xs);
text-align: center;
font-size: var(--font-size-13xl);
color: var(--color-darkslategray);

@media (min-width: 1200px){
    display:none;
    }
`;